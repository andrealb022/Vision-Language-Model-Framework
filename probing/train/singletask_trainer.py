...
class SingleTaskTrainer(BaseTrainer):
    ...
    def build_dataloaders(self):
        ...
        # --- Feature cache: default TRUE se freeze_backbone, con override YAML eventuale ---
        mcfg = self.cfg["model"]
        freeze_bb = bool(mcfg.get("freeze_backbone", True))
        tcfg = self.cfg.get("train", {})
        features_cfg = (tcfg.get("features") or {})
        use_key = features_cfg.get("use", None)
        if use_key is None:
            # default richiesto: usa cache se freeze_backbone
            self.use_feature_cache = freeze_bb
        else:
            # on/off esplicito da YAML
            self.use_feature_cache = (str(use_key).lower() == "true") or (use_key is True)
